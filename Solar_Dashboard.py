...
def generate_initial_data(n=120, start_time=None):
    ...
    # PV power (W) roughly proportional to irradiance with degradation/noise
    base_pv = irradiance * 1.2  # conversion factor to W (tweakable)
    degradation = 1.0  # no long term decay in short sim
    pv_power = base_pv * degradation + np.random.normal(0, 20, n)
    pv_power = np.clip(pv_power, 0, None)
    # consumption W
    consumption = 500 + 200 * np.sin(np.linspace(0, 3.14, n)) + np.random.normal(0, 50, n)
...
def append_simulated_point():
    ...
    base_pv = irr * 1.2
    pv = float(max(0, base_pv + np.random.normal(0, 20)))
    cons = float(max(0, last["consumption_W"] + np.random.normal(0, 30)))
    # SOC change factor adjusted for Watts
    soc = float(np.clip(last["soc_percent"] + (pv - cons) * 0.002 + np.random.normal(0, 0.5), 0, 100))
...
if mode == "Upload Data":
    ...
    st.markdown("CSV should contain at minimum: `timestamp`, `irradiance`, `temperature`, `pv_power_W`, `consumption_W`, `soc_percent`")
...
if mode == "Live Monitor":
    ...
    kpi1.metric("PV Generation (W)", f"{last['pv_power_W']:.1f}")
    kpi2.metric("Consumption (W)", f"{last['consumption_W']:.1f}")
    kpi3.metric("Battery SOC (%)", f"{last['soc_percent']:.1f}")
    kpi4.metric("Export to Grid (W)", f"{last['export_W']:.1f}")
    ...
    fig.update_layout(legend=dict(orientation="h"), margin=dict(t=30, b=10), yaxis_title="Power (W)")
...
# ---------------------------
    ...
    if use_uploaded and uploaded_file:
        ...
        baseline_peak = st.number_input("Baseline peak power (W) at start", value=3000.0, step=100.0)
    ...
    fig.update_layout(height=450, margin=dict(t=30), yaxis_title="Peak Power (W)")
    st.plotly_chart(fig, use_container_width=True)

    st.markdown("### Cell life estimates & recommendations")
    col_a, col_b = st.columns(2)
    col_a.metric("Initial peak (W)", f"{initial_power:.1f}")
    col_b.metric(f"Predicted after {forecast_years} years (W)", f"{future_pred[-1]:.1f}")
    ...
    if crossing_idx is not None:
        ...
        st.warning(f"Predicted to fall to {threshold_pct}% ({threshold_value:.1f} W) on approx **{crossing_date.date()}**")
    else:
...
# ---------------------------
    ...
    st.markdown("Upload CSV with columns `timestamp, irradiance, temperature, pv_power_W, consumption_W, soc_percent`")
    ...
    if uploaded:
        try:
            ...
            # quick plot
            fig = px.line(df, x="timestamp", y=["pv_power_W", "consumption_W"], labels={"value": "Power (W)"})
            st.plotly_chart(fig, use_container_width=True)
