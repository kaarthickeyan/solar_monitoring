...
def generate_initial_data(n=120, start_time=None):
    ...
    # PV power (W) for tiny panel: area~0.01 m^2, eff~15% => ~0.0015 W per W/m^2
    base_pv = irradiance * 0.0015
    degradation = 1.0  # no long term decay in short sim
    pv_power = base_pv * degradation + np.random.normal(0, 0.05, n)
    pv_power = np.clip(pv_power, 0, None)
    # consumption W (sub-watt baseline for tiny load)
    consumption = 0.3 + 0.2 * np.sin(np.linspace(0, 3.14, n)) + np.random.normal(0, 0.05, n)
...
def append_simulated_point():
    ...
    base_pv = irr * 0.0015
    pv = float(max(0, base_pv + np.random.normal(0, 0.05)))
    cons = float(max(0, last["consumption_W"] + np.random.normal(0, 0.03)))
    # scale SOC impact for small W values
    soc = float(np.clip(last["soc_percent"] + (pv - cons) * 0.2 + np.random.normal(0, 0.5), 0, 100))
...
if mode == "Upload Data":
    ...
    st.markdown("CSV should contain at minimum: `timestamp`, `irradiance`, `temperature`, `pv_power_W`, `consumption_W`, `soc_percent` (legacy: pv_power_kW, consumption_kW)")
...
if mode == "Live Monitor":
    ...
    kpi1.metric("PV Generation (W)", f"{last['pv_power_W']:.0f}")
    kpi2.metric("Consumption (W)", f"{last['consumption_W']:.0f}")
    kpi3.metric("Battery SOC (%)", f"{last['soc_percent']:.1f}")
    kpi4.metric("Export to Grid (W)", f"{last['export_W']:.0f}")

    st.markdown("### Generation vs Consumption (last 3 hours)")
    # Timeseries plot
    show_df = df.copy().tail(180)
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=show_df["timestamp"], y=show_df["pv_power_W"], name="PV Generation (W)", mode="lines"))
    fig.add_trace(go.Scatter(x=show_df["timestamp"], y=show_df["consumption_W"], name="Consumption (W)", mode="lines"))
    ...
    fig2.add_trace(go.Bar(x=show_df["timestamp"], y=show_df["export_W"], name="Export W", yaxis="y2", opacity=0.6))
    fig2.update_layout(yaxis=dict(title="SOC %"), yaxis2=dict(title="Export (W)", overlaying="y", side="right"))
...
# ---------------------------
    ...
    - Upload historical data (timestamp, pv_power_W).  
    ...
    use_uploaded = st.checkbox("Use uploaded data (timestamp,pv_power_W)", value=False)
    hist_df = None
    if use_uploaded and uploaded_file:
        try:
            d = pd.read_csv(uploaded_file, parse_dates=["timestamp"])
            # accept W (preferred) or legacy kW and convert to W
            if "pv_power_W" in d.columns:
                hist_df = d[["timestamp", "pv_power_W"]].copy()
                hist_df = hist_df.rename(columns={"pv_power_W": "peak_power_W"})
            elif "pv_power_kW" in d.columns:
                tmp = d[["timestamp", "pv_power_kW"]].copy()
                tmp["peak_power_W"] = tmp["pv_power_kW"] * 1000.0
                hist_df = tmp[["timestamp", "peak_power_W"]]
            else:
                raise ValueError("CSV missing pv_power_W or pv_power_kW")
        ...
        # synthetic baseline peak W and linear degradation per year
        baseline_peak = st.number_input("Baseline peak power (W) at start", value=50.0, step=5.0)
        ...
        for i in range(days):
            ...
            noise = np.random.normal(0, baseline_peak * 0.02)
    ...
    # unify column names
    if "timestamp" in df_hist.columns and "pv_power_W" in df_hist.columns:
        df_hist = df_hist.rename(columns={"timestamp": "date", "pv_power_W": "peak_power_W"})
    ...
    df_hist = df_hist.groupby("date", as_index=False)["peak_power_W"].max()
    ...
    st.metric("Model MAE (W)", f"{mae:.2f}")
    ...
    fig.add_annotation(
        x=last_date,
        y=df_hist["peak_power_W"].max(),  # or another Y position
        text="Today",
        showarrow=False,
        yshift=10
    )
    ...
    col_a.metric("Initial peak (W)", f"{initial_power:.0f}")
    col_b.metric(f"Predicted after {forecast_years} years (W)", f"{future_pred[-1]:.0f}")
    ...
    if crossing_idx is not None:
        ...
        st.warning(f"Predicted to fall to {threshold_pct}% ({threshold_value:.0f} W) on approx **{crossing_date.date()}**")
    else:
...
# ---------------------------
    ...
    st.markdown("Upload CSV with columns `timestamp, irradiance, temperature, pv_power_W, consumption_W, soc_percent` (legacy kW also supported)")
    ...
    if uploaded:
        try:
            ...
            # quick plot with W preferred, kW fallback
            if {"pv_power_W", "consumption_W"}.issubset(df.columns):
                fig = px.line(df, x="timestamp", y=["pv_power_W", "consumption_W"], labels={"value": "W"})
                st.plotly_chart(fig, use_container_width=True)
            elif {"pv_power_kW", "consumption_kW"}.issubset(df.columns):
